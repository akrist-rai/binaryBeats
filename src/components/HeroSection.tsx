...
export const HeroSection = ({ xp, total, playSound, onNavigateTab }: HeroSectionProps) => {
  ...
  return (
    <motion.div className="border-b border-white/[0.08] pb-8 mb-8"
      initial="hidden" animate="visible"
      variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.08 } } }}>

      <motion.div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-6"
        ...
        <div>
          <div className="flex items-center gap-2 font-mono text-[10px] text-zinc-600 tracking-wider uppercase mb-3">
            <span>Binary Beats</span><span className="text-zinc-800">/</span><span className="text-zinc-400">Problem Set</span>
          </div>
          <h1 className="text-4xl md:text-[46px] font-heading font-extrabold tracking-tight leading-[1.05] gradient-text-cool">Problems</h1>
          <p className="text-sm text-zinc-500 mt-2">Real CF problems · local C++17 judge · open-r1/codeforces dataset</p>
        </div>
      ...
      <motion.div className="flex items-center justify-between mb-6 pt-5 border-t border-white/[0.06]"
        variants={{ hidden:{opacity:0}, visible:{opacity:1} }}>
        <div className="flex items-center gap-1.5 text-[10px] font-mono">
          <span className="text-zinc-600">Total XP</span><span className="text-[#c3f73a] font-bold">{xp}</span>
        </div>
        <div className="flex items-center gap-1.5 text-[10px] font-mono">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_1px_220px_1px_220px] gap-0">
          ...
          {cfHandle && cfUser ? (
            <>
              <div className="flex items-center gap-2 mb-3">
                <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#c3f73a]/20 to-[#35e8ff]/10 border border-[#c3f73a]/20 flex items-center justify-center text-sm font-bold text-[#c3f73a] font-mono">
                  {cfHandle[0].toUpperCase()}
                </div>
                <div>
                  <div className="text-sm font-bold text-white">{cfHandle}</div>
                  <div className="text-[10px] font-mono text-zinc-600">{cfUser.rank ?? 'Unrated'}</div>
                </div>
              </div>
              <div className="mb-4">
                <div className="flex items-end gap-2 mb-1">
                  <motion.span className="text-5xl font-mono font-black text-white leading-none"
                    initial={{opacity:0,scale:0.75}} animate={{opacity:1,scale:1}} transition={{delay:0.2,type:'spring'}}>
                    {cfUser.rating ?? '—'}
                  </motion.span>
                  {ratingDelta !== null && (
                    <span className={`text-sm font-mono mb-1.5 font-bold ${ratingDelta>=0?'text-[#c3f73a]':'text-rose-400'}`}>
                ...
                <div className="flex items-end gap-[3px] h-8">
                  {sparkBars.map((h,i) => (
                    <motion.div key={i} className="flex-1 rounded-sm"
                      style={{height:`${h}%`,background:i===sparkBars.length-1?'#c3f73a':`rgba(195,247,58,${0.15+i*0.04})`,originY:1}}
            ...
            <div className="flex flex-col items-start justify-center h-full min-h-[160px]">
              <p className="text-sm text-zinc-500 mb-3 max-w-[220px] leading-relaxed">
        ...
        <motion.div className="px-8 pt-6 md:pt-0" variants={{ hidden:{opacity:0,y:24}, visible:{opacity:1,y:0} }}>
          <div className="text-[10px] font-mono tracking-wider uppercase text-zinc-600 mb-4">Dataset</div>
          <motion.div className="text-2xl font-mono font-black text-white" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{delay:0.5}}>
            {total.toLocaleString()}
          </motion.div>
          <div className="text-[9px] font-mono text-zinc-600 uppercase tracking-wider mt-0.5 mb-4">problems available</div>
          <div className="pt-4 border-t border-white/[0.06] flex items-center justify-between">
            <div>
              <div className="text-2xl font-mono font-black text-[#35e8ff]">{cfSolvedCount}</div>
              <div className="text-[9px] font-mono text-zinc-600 uppercase tracking-wider mt-0.5">CF solved</div>
            </div>
            <div className="text-right">
              <div className="text-2xl font-mono font-black text-[#c3f73a]">{xp}</div>
              <div className="text-[9px] font-mono text-zinc-600 uppercase tracking-wider mt-0.5">total XP</div>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div className="px-8 pt-6 md:pt-0" variants={{ hidden:{opacity:0,x:28}, visible:{opacity:1,x:0,transition:{duration:0.55}} }}>
          ...
          <div className="flex items-center gap-3 mb-4">
            <motion.div className="text-4xl font-mono font-black text-white leading-none"
          ...
          <div className="flex gap-1 mb-4">
            {weekActivity.map((d,i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <div className={`w-full aspect-square rounded-sm ${d.solved?'bg-[#c3f73a]':'bg-white/[0.06]'}`} />
          ...
          {recentActivity.length === 0
            ? <div className="text-[10px] font-mono text-zinc-700">No solves logged yet.</div>
