          ...
          <div className="w-full max-w-[1400px] mx-auto px-6 lg:px-8 py-10 flex-1 flex flex-col">
            <motion.div key="dashboard" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="flex flex-col gap-0">
              ...
              <div className="grid grid-cols-3 gap-3 mb-8">
                <Panel className="px-4 py-3.5 flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-bb-success/10 border border-bb-success/40 flex items-center justify-center text-bb-success text-xs font-bold shrink-0">✓</div>
                  <div className="min-w-0">
                    <StatNumeral value={solvedKeys.length} size="sm" className="text-bb-ink leading-none" />
                    <div className="text-[10px] font-mono uppercase tracking-wider text-bb-ink-faint mt-1">Solved</div>
                  </div>
                </Panel>
                <Panel className="px-4 py-3.5 flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-bb-yellow-fill border border-bb-yellow/40 flex items-center justify-center text-bb-yellow text-xs font-bold shrink-0">🔥</div>
                  <div className="min-w-0">
                    <StatNumeral value={currentStreak} size="sm" className="text-bb-ink leading-none" />
                ...
                <Panel className="px-4 py-3.5 flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <StatNumeral value={total} size="sm" className="text-bb-ink leading-none" />
              ...
              <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
                <Eyebrow tone="muted">Browse</Eyebrow>
                <div className="flex rounded border border-bb-line bg-bb-surface p-0.5 font-mono text-[11px] gap-0.5">
                  {([{ id: 'list' as const, label: 'List' }, { id: 'orbit' as const, label: '◎ Orbit' }]).map(v => (
                    <button key={v.id} onClick={() => { playSound('click'); setViewMode(v.id); }}
                      className={`px-4 h-7 rounded-sm font-bold cursor-pointer transition-colors ${viewMode===v.id?'bg-bb-ink text-bb-ground':'text-bb-ink-faint hover:text-bb-ink-soft'}`}>
                      {v.label}
                    </button>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-8 items-start">
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2.5">
                    <div className="relative flex-1">
                      ...
                      <input ref={searchInputRef} value={search} onChange={e => setSearch(e.target.value)} placeholder="Search problems…"
                        className="w-full h-9 pl-9 pr-9 rounded text-xs font-mono text-bb-ink placeholder-bb-ink-faint focus:outline-none bg-bb-surface border border-bb-line focus:border-bb-line-strong transition-colors" />
                      <kbd className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[9px] font-mono text-bb-ink-faint border border-bb-line rounded-sm px-1 py-0.5 pointer-events-none select-none">/</kbd>
                    </div>
                    <div className="flex rounded border border-bb-line bg-bb-surface p-0.5 font-mono text-[11px] gap-0.5">
                      {(['','easy','medium','hard'] as const).map(d => (
                        <button key={d} onClick={() => { playSound('click'); setDifficulty(d); }}
                          className={`px-3 h-7 rounded-sm font-bold cursor-pointer transition-colors ${difficulty===d?'bg-bb-ink text-bb-ground':'text-bb-ink-faint hover:text-bb-ink-soft'}`}>
                    ...
                    <div className="flex rounded border border-bb-line bg-bb-surface p-0.5 font-mono text-[11px] gap-0.5">
                      {([{id:'all' as const,label:'All'},{id:'unsolved' as const,label:'Todo'},{id:'solved' as const,label:'Done'}]).map(s => (
                        <button key={s.id} onClick={() => { playSound('click'); setStatusFilter(s.id); }}
                          className={`px-3 h-7 rounded-sm font-bold cursor-pointer transition-colors ${statusFilter===s.id?'bg-bb-ink text-bb-ground':'text-bb-ink-faint hover:text-bb-ink-soft'}`}>
                          {s.label}
                        </button>
                      ))}
                    </div>
                    <select value={sortBy} onChange={e => { playSound('click'); setSortBy(e.target.value as typeof sortBy); }}
                      className="h-9 px-3 rounded text-[11px] font-mono font-bold text-bb-ink-soft bg-bb-surface border border-bb-line focus:outline-none focus:border-bb-line-strong cursor-pointer">
                    ...
                    <div className="flex items-center gap-2 flex-wrap -mt-1">
                      {selectedTag && (
                        <button onClick={() => setSelectedTag(null)} className="cursor-pointer">
                          <Tag tone="accent">#{selectedTag} ×</Tag>
                        </button>
                      )}
                      {activeFilterCount > 0 && (
                        <button onClick={resetFilters}
                          className="text-[10px] font-mono uppercase tracking-wider text-bb-ink-faint hover:text-bb-yellow transition-colors cursor-pointer underline underline-offset-2">
                          Clear all filters
                        </button>
                      )}
                    </div>
                  ...
                  {/* Column headers */}
                  <div className="grid grid-cols-[32px_1fr_90px_80px] gap-2 items-center px-4 font-mono text-[10px] font-bold uppercase tracking-wider text-bb-ink-faint select-none">
                  ...
                  <div className="flex flex-col gap-1">
                    {loading
                      ? Array.from({length:8}).map((_,i) => <div key={i} className="h-14 rounded bg-bb-ink/[0.02] border border-bb-line animate-pulse" />)
                      : error
                        ? <div className="py-12 text-center text-xs text-bb-danger font-mono border border-dashed border-bb-danger/30 rounded">{error}</div>
                        : displayProblems.length === 0
                          ? <div className="py-16 flex flex-col items-center gap-3 text-center text-xs text-bb-ink-faint font-mono border border-dashed border-bb-line rounded">
                          ...
                          : displayProblems.map((p, i) => {
                              ...
                              const diffTone = diff==='Easy' ? 'success' : diff==='Medium' ? 'accent' : diff==='Hard' ? 'danger' : 'neutral';
                              const accentBar = diff==='Easy' ? 'bg-bb-success' : diff==='Medium' ? 'bg-bb-yellow' : diff==='Hard' ? 'bg-bb-danger' : 'bg-bb-line-strong';
                              return (
                                <motion.div key={p.key} layout
                                  ...
                                  className="relative grid grid-cols-[32px_1fr_90px_80px] gap-2 items-center pl-5 pr-4 py-3.5 rounded border cursor-pointer transition-all group border-bb-line bg-bb-surface hover:border-bb-line-strong overflow-hidden">
                                  <span className={`absolute left-0 top-0 bottom-0 w-[3px] ${accentBar}`} />
                                  <span className="link-chip">↗</span>
                                  <div className="flex justify-center">
                                    {isSolved ? (
                                      <div className="w-4 h-4 rounded-full bg-bb-success/15 border border-bb-success flex items-center justify-center text-bb-success text-[10px] font-mono font-bold select-none">✓</div>
                                    ) : (
                                      <div className="w-4 h-4 rounded-full border border-bb-line-strong group-hover:border-bb-yellow transition-colors" />
                                    )}
                                  </div>
                                  <div className="min-w-0">
                                    <div className="flex items-center gap-2 mb-1">
                                      <span className="text-[10px] font-mono text-bb-ink-faint tabular-nums shrink-0">{p.contestId}{p.index}</span>
                                      <span className={`text-sm font-medium truncate transition-colors ${isSolved ? 'text-bb-ink-faint line-through' : 'text-bb-ink group-hover:text-bb-yellow'}`}>{p.title ?? p.key}</span>
                                      {p.judgeable && <Tag tone="success" className="shrink-0">judge</Tag>}
                                    </div>
                                    <div className="flex items-center gap-1.5 flex-wrap">
                                      {p.tags.slice(0,4).map(t => (
                                        <button key={t} onClick={e => { e.stopPropagation(); playSound('click'); setSelectedTag(selectedTag===t?null:t); }} className="cursor-pointer">
                ...
                <div className="flex flex-col gap-4">
                  ...
                  {nextUp && (
                    <Panel bracket className="p-5 relative overflow-hidden text-bb-yellow">
                      <div className="absolute -right-8 top-3 rotate-45 bg-bb-yellow text-bb-ground text-[9px] font-mono font-bold uppercase tracking-widest px-8 py-0.5 select-none pointer-events-none">
                        Pick
                      </div>
                      <Eyebrow tone="accent" className="mb-3">Next Up</Eyebrow>
                      <div className="text-[10px] font-mono text-bb-ink-faint tabular-nums mb-1">{nextUp.contestId}{nextUp.index}</div>
                      <div className="text-base font-display font-bold text-bb-ink mb-3 leading-snug pr-6">{nextUp.title ?? nextUp.key}</div>
                      <div className="flex items-center gap-2 mb-4 flex-wrap">
                        <RatingBadge rating={nextUp.rating} />
                        {nextUp.tags[0] && (
                          <Tag tone="neutral">{nextUp.tags[0]}</Tag>
                        )}
                      </div>
                      <Button variant="primary" size="md" className="w-full" onClick={() => open(nextUp.key)} onMouseEnter={() => playSound('hover')}>
                        Solve →
                      </Button>
                    </Panel>
                  )}
                  ...
                  <Panel className="p-5">
                    <div className="flex items-center justify-between mb-3">
                      <Eyebrow tone="muted">Filter by Topic</Eyebrow>
                      {selectedTag && (
                        <button onClick={() => setSelectedTag(null)} className="text-[9px] font-mono uppercase tracking-wider text-bb-ink-faint hover:text-bb-yellow transition-colors cursor-pointer">
                          Clear
                        </button>
                      )}
                    </div>
                      ...
                      {CF_TAGS.map(t => {
                        ...
                        return (
                          <button key={t} onClick={() => { playSound('click'); setSelectedTag(active ? null : t); }}
                            className={`px-2.5 py-1.5 rounded text-[10.5px] font-mono cursor-pointer transition-all border ${active?'bg-bb-ink text-bb-ground border-bb-ink':'text-bb-ink-soft hover:text-bb-ink hover:bg-bb-ink/[0.03] border-bb-line'}`}>
                            {t}
                          </button>
                        );
                      })}
                    ...
                    <div className="pt-4 border-t border-dashed border-bb-line">
                      <div className="flex items-center justify-between mb-2.5">
                        <Eyebrow tone="muted">This View</Eyebrow>
                        <span className="text-[10px] font-mono text-bb-ink-faint tabular-nums">{displayProblems.length} shown</span>
                      </div>
                      <div className="flex h-2 rounded-sm overflow-hidden bg-bb-ink/[0.05] mb-2.5">
                        {(['Easy','Medium','Hard','Unrated'] as const).map(d => {
                          ...
                          const bg = d==='Easy' ? 'bg-bb-success' : d==='Medium' ? 'bg-bb-yellow' : d==='Hard' ? 'bg-bb-danger' : 'bg-bb-line-strong';
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        {(['Easy','Medium','Hard','Unrated'] as const).map(d => (
                          <span key={d} className="flex items-center gap-1.5 text-[10px] font-mono text-bb-ink-soft">
                            <span className={`w-1.5 h-1.5 rounded-full ${d==='Easy'?'bg-bb-success':d==='Medium'?'bg-bb-yellow':d==='Hard'?'bg-bb-danger':'bg-bb-line-strong'}`} />
                  ...
                  <Panel className="p-5">
                          ...
                          {Array.from({length:7}, (_,day) => {
                            ...
                            return (
                              <div key={day} className="w-[9px] h-[9px] rounded-[1.5px] cursor-pointer hover:ring-1 hover:ring-bb-ink/30 transition-all relative group/cell"
                                style={{background:SCOREBOARD_HEAT[log.val]}}>
                                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 w-24 hidden group-hover/cell:block bg-bb-ink text-bb-ground border border-bb-ink text-[8px] p-1.5 rounded-sm shadow-sticker-sm text-center pointer-events-none z-50">
                                  {log.date}
                                </div>
                              </div>
                            );
                    ...
                    <div className="flex items-center justify-between mt-3">
                      ...
                      <div className="flex gap-1">
                        {SCOREBOARD_HEAT.map((c,i) => <div key={i} className="w-2 h-2 rounded-sm" style={{background:c}} />)}
                      </div>
