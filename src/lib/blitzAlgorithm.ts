...
/**
 * Solo Blitz: a 4-problem staircase around the player's own rating, from
 * slightly-below (a confidence opener) up to a stretch problem.
 */
 ...
 * higher-rated player's pull capped at +200. This keeps the set achievable for
 * the weaker player; fairness for the stronger player comes from the
 * first-to-solve "claim" race, not from harder problems.
 */
 ...
 * search window around a target if nothing matches exactly. Throws
  ...
  for (const target of targets) {
    ...
    for (const widen of WIDEN_STEPS) {
      candidates = catalog.filter((p) => {
        ...
        return (
          Math.abs(p.rating - target) <= widen &&
          !solvedKeys.has(key) &&
          !pickedKeys.has(key)
        );
      });
    ...
    picked.push({ contestId: chosen.contestId, index: chosen.index, name: chosen.name, rating: chosen.rating });
